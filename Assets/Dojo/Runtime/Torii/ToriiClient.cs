...
namespace Dojo.Torii
{
  public unsafe class ToriiClient
  {
    ...
    // Static fields to store dispatch settings
    private static bool entityUpdateDispatchToMainThread;
    private static bool eventMessageUpdateDispatchToMainThread;
    private static bool tokenUpdateDispatchToMainThread;
    private static bool tokenBalanceUpdateDispatchToMainThread;

    [MonoPInvokeCallback(typeof(dojo.FnPtr_CString_Void.@delegate))]
    private static void LogCallback(CString msg)
    {
        Debug.Log(msg);
    }

    [MonoPInvokeCallback(typeof(dojo.FnPtr_FieldElement_CArrayStruct_Void.@delegate))]
    private static void EntityStateUpdateCallback(dojo.FieldElement key, dojo.CArrayStruct models)
    {
        ...
        if (entityUpdateDispatchToMainThread)
        {
    ...
    [MonoPInvokeCallback(typeof(dojo.FnPtr_FieldElement_CArrayStruct_Void.@delegate))]
    private static void EventMessageUpdateCallback(dojo.FieldElement key, dojo.CArrayStruct models)
    {
        var mappedModels = new Model[(int)models.data_len];
        for (var i = 0; i < (int)models.data_len; i++)
        {
        ...
        if (eventMessageUpdateDispatchToMainThread)
        {
    ...
    [MonoPInvokeCallback(typeof(dojo.FnPtr_Token_Void.@delegate))]
    private static void TokenUpdateCallback(dojo.Token token)
    {
        var mappedToken = new Token(
            new FieldElement(token.contract_address), 
            new BigInteger(token.token_id.data, false, true), 
            token.name, 
            token.symbol, 
            token.decimals, 
            JsonConvert.DeserializeObject<Dictionary<string, object>>(token.metadata)
        );
        
        Action emit = () => ToriiEvents.Instance.TokenUpdated(mappedToken);
        
        if (tokenUpdateDispatchToMainThread)
        {
    ...
    [MonoPInvokeCallback(typeof(dojo.FnPtr_TokenBalance_Void.@delegate))]
    private static void TokenBalanceUpdateCallback(dojo.TokenBalance balance)
    {
        var mappedTokenBalance = new TokenBalance(
            new BigInteger(balance.balance.data, false, true), 
            new FieldElement(balance.account_address), 
            new FieldElement(balance.contract_address), 
            new BigInteger(balance.token_id.data, false, true)
        );
        
        Action emit = () => ToriiEvents.Instance.TokenBalanceUpdated(mappedTokenBalance);
        
        if (tokenBalanceUpdateDispatchToMainThread)
        {
            UnityMainThreadDispatcher.Instance().Enqueue(emit);
        }
        else
    ...
    public ToriiClient(string toriiUrl, string relayUrl, FieldElement worldAddress, bool dispatchEventsToMainThread = true)
    {
      ...
      client = result._ok;
      dojo.client_set_logger(client, new dojo.FnPtr_CString_Void(LogCallback));
    ...
    {
      entityUpdateDispatchToMainThread = dispatchToMainThread;
      onEntityStateUpdate = EntityStateUpdateCallback;
    ...
    public void RegisterTokenUpdateEvent(FieldElement[] contractAddresses = null, BigInteger[] tokenIds = null, bool dispatchToMainThread = true)
    {
      ...
      tokenUpdateDispatchToMainThread = dispatchToMainThread;
    ...
    public void RegisterTokenBalanceUpdateEvent(FieldElement[] contractAddresses = null, FieldElement[] accountAddresses = null, BigInteger[] tokenIds = null, bool dispatchToMainThread = true)
    {
      ...
      tokenBalanceUpdateDispatchToMainThread = dispatchToMainThread;
      onTokenBalanceUpdate = TokenBalanceUpdateCallback;
    ...
    {
      eventMessageUpdateDispatchToMainThread = dispatchToMainThread;
      onEventMessagesUpdate = EventMessageUpdateCallback;
