...
./CAstFfi/bin/CAstFfi.Tool/Release/net8.0/CAstFfi.Tool extract --config Bindings/config-extract-linux.json
if [ $? -ne 0 ]; then
    print_error "Failed to extract Linux bindings"
    exit 1
fi

./CAstFfi/bin/CAstFfi.Tool/Release/net8.0/CAstFfi.Tool extract --config Bindings/config-extract-macos.json > /dev/null 2>&1
if [ $? -ne 0 ]; then
    print_error "Failed to extract macOS bindings"
    exit 1
fi

./CAstFfi/bin/CAstFfi.Tool/Release/net8.0/CAstFfi.Tool extract --config Bindings/config-extract-windows.json > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
./CAstFfi/bin/CAstFfi.Tool/Release/net8.0/CAstFfi.Tool merge --inputDirectoryPath Bindings/ast --outputFilePath Bindings/ast/cross-platform.json > /dev/null 2>&1
if [ $? -ne 0 ]; then
